...
export default function Projects() {
    ...
    // Generate previews (object URLs for images, text snippet for text files)
    React.useEffect(() => {
        let revoked: string[] = []
        async function gen() {
    ...
    async function handleUploadProject() {
        ...
        try {
            ...
            // Convert to ProjectFile shape for API methods
            const projectFiles = fileMetas.map(f => ({
                id: f.id,
                name: f.name,
                type: f.type,
                dataUrl: f.dataUrl || '',
                            ...
                            <div style={{
                                ...
                                {previews.map((p, idx) => (
                                    <div key={idx} style={{
                                        ...
                                        {p.url && (
                                            <img src={p.url} alt={p.name} style={{ width: '100%', height: '140px', objectFit: 'cover', borderRadius: '6px', marginTop: '0.5rem' }} />
                                        )}
                                        {p.text && (
                                            <pre style={{
                                                marginTop: '0.5rem',
